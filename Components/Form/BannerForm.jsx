...
const GetSubject = () => {
  const { response } = useFetch('post',`${process.env.NEXT_PUBLIC_API_URL}/customer/SubjectById/${selectedService.id}`)
...
useEffect( () => {
  cat.data.map(item => {
    optionsServices.push({value: item.slug , label: item.name})
  })
