        ...
        <tbody>
          {OrderList.map((item , key)=>(
            <tr key={key} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                {cat.data.map(value => (
                  value.id == item.category_id && <td className="py-4 px-6 whitespace-nowrap">{value.name}</td>
                ))}
                {subWithCat.data.map(value => (
                  value.id == item.subject_id && <td className="py-4 px-6 whitespace-nowrap">{value.name}</td>
                ))}
